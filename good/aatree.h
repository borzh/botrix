...
namespace good
{
	...
	public:
		...
		public:
			...
			/// Pre-increment.
			iterator& operator++() { DebugAssert(m_pCurrent); m_pCurrent = _get_next(m_pCurrent, 1); return *this; }
			/// Pre-decrement.
			iterator& operator--() { DebugAssert(m_pCurrent); m_pCurrent = _get_next(m_pCurrent, 0); return *this; }

			/// Post-increment.
			iterator operator++(int) { DebugAssert(m_pCurrent); iterator tmp(*this); m_pCurrent = _get_next(m_pCurrent, 1); return tmp; }
			/// Post-decrement.
			iterator operator--(int) { DebugAssert(m_pCurrent); iterator tmp(*this); m_pCurrent = _get_next(m_pCurrent, 0); return tmp; }

			/// Dereference.
			T& operator*() const { DebugAssert(m_pCurrent); return m_pCurrent->elem; }
			/// Element selection through pointer.
			T* operator->() const { DebugAssert(m_pCurrent); return &m_pCurrent->elem; }
		};
